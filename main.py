...
def _create_weather_summary(
    forecasts, weather_monitor, use_emoji=True, claude_config=None
):
            ...
            for days in range(0, 3):
                ...
                if daily:
                    day_name = (
                        "Today"
                        if days == 0
                        else (now + timedelta(days=days)).strftime("%A")
                    )

                    # build day description with trends
                    day_desc = (
                        f"{day_name}: {daily['temp_min']:.0f}-{daily['temp_max']:.0f}°C"
                    )
                    ...
                    if prev_wind is not None and abs(wind_gust - prev_wind) >= 15:
                        wind_trend = (
                            "increasing" if wind_gust > prev_wind else "decreasing"
                        )
        ...
        for days in range(0, 4):
            ...
            if daily:
                if days == 0:
                    ...
                    day_name = (
                        (datetime.now() + timedelta(days=days)).strftime("%A").upper()
                    )
                ...
                lines.append(
                    f"🌡️ <b>High {high_temp:.0f}°C</b> {temp_emoji} • <b>Low {low_temp:.0f}°C</b>"
                )

                # visual temperature bar
                temp_bar = _create_temp_bar(low_temp, high_temp)
                lines.append(
                    f"<code>{temp_bar}</code> <i>{low_temp:.0f}° → {high_temp:.0f}°</i>"
                )
                lines.append("")
                ...
                if daily["precipitation_total"] > 0:
                    ...
                    lines.append(
                        f"{precip_emoji} <b>{precip:.1f} mm</b> <i>({intensity})</i>"
                    )
...
async def check_weather_and_send_alerts(
    ...
    try:
        ...
        notifier = TelegramNotifier(
            bot_token=telegram_config["bot_token"], chat_ids=all_chat_ids
        )

        use_emoji = telegram_config.get("message_format", {}).get("include_emoji", True)

        # always send weather summary
        logger.info("sending weather summary...")
        claude_config = config_loader.get_claude_config()
        summary_message = _create_weather_summary(
            forecasts, weather_monitor, use_emoji, claude_config
        )
        results = await notifier.send_message(summary_message, parse_mode="HTML")

        # send hourly forecast as separate messages for each location
        logger.info("sending hourly forecasts...")
        for forecast in forecasts:
            hourly_section = HourlyForecastFormatter.format_hourly_forecast(forecast)
            if hourly_section:
                hourly_results = await notifier.send_message(
                    hourly_section, parse_mode="HTML"
                )
                results["success"].extend(hourly_results["success"])
                results["failed"].extend(hourly_results["failed"])

        # send alerts if any (in addition to summary)
        if alerts:
            logger.info(f"sending {len(alerts)} alert(s) via telegram...")
            alert_results = await notifier.send_alerts(alerts, use_emoji=use_emoji)
            # merge alert results with summary results
            results["success"].extend(alert_results["success"])
            results["failed"].extend(alert_results["failed"])
...
def main():
    ...
    parser.add_argument(
        "--dev",
        action="store_true",
        help="send only to your chat ID (1216738537) for testing",
    )
    ...
    # If --dev flag is set, use your chat ID
    if args.dev:
        args.dev_chat_id = "1216738537"
    ...
    alert_count = asyncio.run(
        check_weather_and_send_alerts(
            config_loader, verbose=args.verbose, dev_chat_id=args.dev_chat_id
        )
    )
